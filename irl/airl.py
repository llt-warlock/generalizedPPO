...
class DiscriminatorMLP(nn.Module):
    def __init__(self, state_shape, in_channels):
...
class Discriminator(nn.Module):
    def __init__(self, state_shape, in_channels=6, latent_dim=None):
        ...
        # Layers
        self.reward_conv1 = nn.Conv2d(in_channels=self.in_channels, out_channels=32, kernel_size=3, padding=1)
        self.reward_conv2 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, padding=1)
        self.reward_conv3 = nn.Conv2d(in_channels=32, out_channels=16, kernel_size=3, padding=1)
        self.reward_out = nn.Linear(16 * state_shape[0] * state_shape[1], 1)

        self.value_conv1 = nn.Conv2d(in_channels=self.in_channels, out_channels=32, kernel_size=3, padding=1)
        self.value_conv2 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, padding=1)
        self.value_conv3 = nn.Conv2d(in_channels=32, out_channels=16, kernel_size=3, padding=1)
        self.value_out = nn.Linear(16 * state_shape[0] * state_shape[1], 1)


        # # Layers
        # self.reward_conv1 = nn.Conv2d(in_channels=self.in_channels, out_channels=32, kernel_size=3, padding=1)
        # self.reward_conv2 = nn.Conv2d(in_channels=32, out_channels=64, kernel_size=3, padding=1)
        # self.reward_conv3 = nn.Conv2d(in_channels=64, out_channels=32, kernel_size=3, padding=1)
        # self.reward_out = nn.Linear(32 * state_shape[0] * state_shape[1], 1)
        #
        # self.value_conv1 = nn.Conv2d(in_channels=self.in_channels, out_channels=32, kernel_size=3, padding=1)
        # self.value_conv2 = nn.Conv2d(in_channels=32, out_channels=64, kernel_size=3, padding=1)
        # self.value_conv3 = nn.Conv2d(in_channels=64, out_channels=32, kernel_size=3, padding=1)
        # self.value_out = nn.Linear(32 * state_shape[0] * state_shape[1], 1)
...
def update_discriminator(discriminator, optimizer, gamma, expert_trajectories, policy_trajectories, ppo, batch_size, epcohes,
                         latent_posterior=None):

    for epoch in range(epcohes):
        ...
        #print("label prediction : ", label_predictions)
