...
class Discriminator(nn.Module):
    ...
    def g(self, state, latent=None):
        # ... same as before but add BatchNorm and possibly Dropout after each activation
        x = self.relu(self.bn1(self.reward_conv1(state)))
        x = self.dropout(x)
        x = self.relu(self.bn2(self.reward_conv2(x)))
        x = self.dropout(x)
        x = self.relu(self.bn3(self.reward_conv3(x)))
        x = x.view(x.shape[0], -1)
        x = self.reward_out(x)
        return x
...
def update_discriminator(discriminator, optimizer, gamma, expert_trajectories, policy_trajectories, ppo, batch_size, epochs,
