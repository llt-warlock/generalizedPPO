...
from adaption.generalization_ppo.evaluation import evaluate_three_obj
...
from adaption.generalization_ppo.samping.preference_sampling import generate_samples
import wandb
...
if __name__ == '__main__':
    ...
    weight_vectors = generate_samples(config.n_workers, config.n_objs)
    #weight_vectors = generate_samples_with_granularity(config.n_workers, config.n_objs, 0.05)
    ...
    try:
        for t in tqdm(range(max_step)):
            ...
            if re_sample:
                ...
                #fixed_samples = generate_fixed_samples_two(1)
                #random_samples = generate_samples_two(16, 2)
                weight_vectors = generate_samples(config.n_workers, config.n_objs)
                #weight_vectors = generate_samples_with_granularity(config.n_workers, config.n_objs, 0.05)
            ...
            #print("preferences : ", weight_vectors)
            ...
            if t % config.update_frequency == 0:
                ...
                #evaluate_four_obj(ppo, config)

            # if save_count % 100 == 0:
            #     torch.save(ppo.state_dict(),
            #                '../ppo_model/generalization/v_5/final_version/v_3_25_steps_' + str(save_count) + '.pt')
                # torch.save(ppo.state_dict(),
                #            '../ppo_model/generalization/v_5/sample_005/v_5_40_steps_' + str(save_count) + '.pt')
        ...
        # torch.save(ppo.state_dict(),
        #            '../ppo_model/generalization/v_3/final_version/ttttt_' + timestamp + '.pt')
