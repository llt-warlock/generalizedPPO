...
class PPO_CNN(nn.Module):
    def __init__(self, state_shape, in_channels, n_actions, weight, contexts):
        ...
        self.value_head = nn.Linear(in_features=64, out_features=3)
    ...
    def evaluate_trajectory(self, tau, weights):
        ...
        # action_probabilities, critic_values = self.forward(trajectory_states, weights)
        # _, critic_values_ = self.forward(trajectory_next_states, weights)
...
def update_policy(ppo, dataset, optimizer, gamma, epsilon, n_epochs, entropy_reg, GAE_lambda):
    ...
    for epoch in range(n_epochs):
        ...
        for i, tau in enumerate(dataset.trajectories):
            ...
            # for four objs
            #rewards = reward_vectors_raw[:, 0] * weights[:, 0] + reward_vectors_raw[:, 1] * weights[:, 1] + reward_vectors_raw[:, 2] * weights[:, 2] + reward_vectors_raw[:, 3] * weights[:, 3]
            ...
            # for four objectives
            # weighted_values = weights[:, 0] * critic_values[:, 0] + weights[:, 1] * critic_values[:, 1] + weights[:, 2] * critic_values[:, 2] + weights[:, 3] * critic_values[:, 3]
            # weighted_values_ = weights[:, 0] * critic_values_[:, 0] + weights[:, 1] * critic_values_[:, 1] + weights[:, 2] * critic_values_[:, 2] + weights[:, 3] * critic_values_[:, 3]
        ...
        overall_loss = (batch_loss + value_loss) / (dataset.batch_size * 2)
