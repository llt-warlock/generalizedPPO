...
def random_gen_item(board, occupied):
    remaining_pos = np.setdiff1d(board, occupied)
    if len(remaining_pos) > 0:
        random_choice = np.random.choice(remaining_pos, size=1)
        return v_3_scalar_to_idx(random_choice)
    else:
        random_choice = None
        return
    ...
    def update(self, actions, board, layers, backdrop, things, the_plot):
        ...
        if the_plot.frame == 0:
            if 'okla' in the_plot:
                ...
                random_positions = np.random.choice(BOARD, size=11, replace=False)
                item_A_positions = random_positions[0:5]
                item_B_positions = random_positions[5:10]
                ...
                #print("occupied pos : ", random_positions)
                the_plot['occupied'] = random_positions
...
class ItemADrape(plab_things.Drape):
    ...
    def update(self, actions, board, layers, backdrop, things, the_plot):
        ...
        if actions == 5 and self.curtain[(player_row - 1, player_col)]:  # grab upward?
            ...
            the_plot['occupied'] = np.setdiff1d(the_plot['occupied'], [v_3_idx_to_scalar(player_row - 1, player_col)])
            new_pos = random_gen_item(BOARD, the_plot['occupied'])
            the_plot['occupied'] = np.append(the_plot['occupied'], new_pos)
            self.curtain[new_pos] = True
        ...
        if actions == 7 and self.curtain[(player_row, player_col - 1)]:  # grab leftward?
            ...
            the_plot.add_reward(np.array([1.0, 0.0]))
            the_plot['occupied'] = np.setdiff1d(the_plot['occupied'], [v_3_idx_to_scalar(player_row, player_col - 1)])
            new_pos = random_gen_item(BOARD, the_plot['occupied'])
            the_plot['occupied'] = np.append(the_plot['occupied'], new_pos)
            self.curtain[new_pos] = True
