...
class TrajectoryDataset:
    def __init__(self, batch_size, n_workers):
        ...
        self.buffer = [{'states': [], 'actions': [], 'next_states': [], 'rewards': [], 'dones': [], 'dw': [], 'do': [],
                        ...
                        'leftAmax': None, 'rightAmax':None, 'leftBmax':None, 'rightBmax':None, 'experts':[], 'weights':[], 'count':0}
                       for i in range(n_workers)]


    def remove_first_element_from_buffer(self, i):
        for key, value in self.buffer[i].items():
    ...
    def write_tuple(self, states, actions, next_states, done, log_probs, logs=None, info=None,
        ...
        for i in range(self.n_workers):
            ...
            if self.buffer[i]['count'] > 10:
                self.trajectories.append(self.buffer[i].copy())
                print("trajectory : ", self.buffer[i].copy())
                self.reset_buffer(i)
